...
/* Modified by the MoTou project in 2026: this browser build reuses the
 * Apache-2.0 CFB reader bundled by SheetJS instead of Node.js require(),
 * and decodes UTF-16LE CStrings locally instead of loading codepage. */
var CFB = (typeof globalThis !== 'undefined' && globalThis.XLSX) ? globalThis.XLSX.CFB : undefined;

if(typeof require !== 'undefined') {
	if(typeof CFB === 'undefined') CFB = require('cf'+'b');
}
var parsenoop = function(blob, length) { blob.l += length; }; // 跳过未实现的记录类型，而不是抛出错误
...
function parse_CString(blob, length, opts) {
	var bytes = blob.slice(blob.l, blob.l+length), o = '';
	/* CString is UTF-16LE. Decode locally so the browser build needs no codepage runtime. */
	for(var i = 0; i + 1 < bytes.length; i += 2) o += String.fromCharCode(bytes[i] | (bytes[i + 1] << 8));
	blob.l += length;
	return o;
}
